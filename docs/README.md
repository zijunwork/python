...
List(列表)是Python中使用最频繁的数据类型。列表是写在方括号之间、用逗号分隔开的元素列表。

和字符串比较：

1. 和字符串一样，列表可以被索引、切片、串联
2. 和字符串不同，列表的元素是可以改变的
...
元组（tuple）与列表类似，不同之处在于元组不能修改。元组写在小括号里，元素之间用逗号隔开。
...
+ 与字符串一样，元组的元素不能修改
+ 与字符串一样，元组可以被索引、切片
+ 注意构造包含0和1个元素的元组的特殊语法规则：一个元素，需要在元素后添加逗号
+ 元组也可以使用 `+` 操作符进行拼接
...
字典（dictionaries）是Python中另一个非常有用的内置数据类型。字典是一种映射类型，它是一个无序的键：值对集合。关键字必须使用不可变类型，也就是说list和包含可变类型的tuple不能做关键字。在同一个字典中，关键字还必须互不相同。
...
# 删除一个键值对
del dic['c']
print(dic)  # > {'a': 1, 'b': 2, 'd': 4, 'e': 5, 'f': 6}

# 返回所有键值
print(list(dic.keys()))  # > ['a', 'b', 'd', 'e', 'f']

# 按值排序
print(sorted(dic.values()))  # > [1, 2, 4, 5, 6]
```

==需要注意==：

+ 字典是一种映射类型，它的元素是键值对
+ 字典的关键字必须wi不可变类型，且不能重复
+ 创建空字典使用 `{}`
